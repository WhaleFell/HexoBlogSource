...
1. 对于同一个 Go 文件,从上到下执行  
   ![](https://api.whaleluo.top/onedrive/file/?path=/picstorage/blog/Golang/package-1.png)
2. 对于同一个 package 中的不同文件,将文件名按字符串进行从小到大排序,之后顺序调用各文件中的 `init()` 函数.  
   ![](https://api.whaleluo.top/onedrive/file/?path=/picstorage/blog/Golang/package-2.png)
...
4. 如果 package 存在依赖,调用顺序为最后被依赖的最先被初始化.  
   ...
   ![](https://api.whaleluo.top/onedrive/file/?path=/picstorage/blog/Golang/package-3.png)
